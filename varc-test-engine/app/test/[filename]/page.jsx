...
export default function TestEngine({ params }) {
  ...
  // Layout and Resizing States
  const [leftWidth, setLeftWidth] = useState(50);
  const isDragging = useRef(false);

  // Filters and Context States
  ...
  const onDragMove = (e) => {
    ...
    if (currentX) {
      const newWidth = (currentX / window.innerWidth) * 100;
  ...
  const renderOptionsPane = (qData, qIndex) => {
    ...
    // Dynamically choose options labels based on the answer key observed
    ...
    return (
      <div key={`opts-${qIndex}`} className="sticky top-0 z-20 mb-5 bg-white/95 backdrop-blur-xl rounded-2xl p-4 border border-slate-200 shadow-md">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
             <span className="font-extrabold text-slate-400 uppercase tracking-widest text-[10px] md:text-xs">
              ...
              {optionButtons.map(opt => {
                ...
                return (
                  <button 
                    ...
                    className={`w-9 h-9 md:w-12 md:h-12 rounded-full border-2 text-sm md:text-lg font-extrabold transition-all duration-300 flex items-center justify-center ${btnColor}`}
                  >
              ...
              {optionButtons.map(opt => {
                ...
                return (
                  <button 
                    ...
                    className={`w-9 h-9 md:w-12 md:h-12 rounded-xl border-2 text-sm md:text-lg font-extrabold transition-all duration-300 flex items-center justify-center ${btnColor}`}
                  >
                    {qLocked && isCorrectAnswer ? <Check size={20} strokeWidth={4} /> : (qLocked && isSelected ? <X size={20} strokeWidth={4}/> : opt)}
                  </button>
                )
  ...
  return (
    ...
    >
      ...
      {/* Modern Compact Header (Replacing the separate footer layout) */}
      <header className="text-slate-300 p-2 flex justify-between items-center shrink-0 border-b border-slate-800/80 bg-slate-950 z-30 shadow-md">
        ...
        <div className="flex items-center gap-1.5 md:gap-3">
          ...
          <div className="font-mono bg-slate-900 px-2 py-1.5 md:py-2 rounded-xl text-indigo-400 border border-indigo-500/20 text-xs font-bold shadow-inner flex items-center gap-1.5">
            <Clock size={14} className="animate-pulse" /> {formatTime(timeSpent)}
          </div>
        </div>

        {/* Central Core Navigation */}
        <div className="flex items-center bg-slate-900 rounded-xl border border-slate-700/50 p-1">
          <button
            ...
            className="p-1.5 disabled:opacity-30 hover:bg-slate-800 rounded-lg text-slate-300 transition-colors"
          >
            <ArrowLeft size={16} />
          </button>
          <span className="px-3 font-mono font-bold text-slate-400 tracking-widest text-[10px] md:text-xs">
            {currentIndex + 1}/{data.length}
          </span>
          {currentIndex === data.length - 1 ? (
            <button
              onClick={(e) => { e.stopPropagation(); submitTest(); }}
              className="p-1.5 text-emerald-400 hover:bg-emerald-900/30 rounded-lg transition-colors flex items-center gap-1"
            >
              <span className="hidden md:inline text-xs font-bold">Submit</span>
              <CheckCircle size={16} />
            </button>
          ) : (
            <button
              onClick={(e) => { e.stopPropagation(); setCurrentIndex(prev => prev + 1); }}
              className="p-1.5 hover:bg-slate-800 rounded-lg text-indigo-400 transition-colors flex items-center gap-1"
            >
        ...
        <div className="flex items-center gap-1.5 md:gap-3">
           ...
           <div className="flex items-center gap-1 bg-slate-900 rounded-xl border border-slate-800 p-1 shadow-inner">
              <button onClick={(e) => {e.stopPropagation(); setFontSize(f => Math.max(12, f - 2))}} className="p-1.5 hover:bg-slate-800 rounded-lg text-slate-400 transition-colors"><AArrowDown size={14} /></button>
              <div className="w-px h-4 bg-slate-700"></div>
              <button onClick={(e) => {e.stopPropagation(); setFontSize(f => Math.min(32, f + 2))}} className="p-1.5 hover:bg-slate-800 rounded-lg text-slate-400 transition-colors"><AArrowUp size={14} /></button>
           </div>
        </div>
      </header>
      ...
      <main 
        ...
        className={`flex-1 flex overflow-hidden p-1 md:p-3 gap-1 md:gap-2 ${isSingleColumn ? 'justify-center items-center' : 'flex-col landscape:flex-row lg:flex-row'}`}
      >
        
        {!isSingleColumn && (
          <div className="w-full lg:w-[var(--left-width)] landscape:w-[var(--left-width)] shrink-0 h-[45%] lg:h-full landscape:h-full bg-slate-50 md:rounded-2xl shadow-inner overflow-y-auto p-4 md:p-8 scrollbar-thin relative group">
            <div 
        ...
        {!isSingleColumn && (
          <div
            ...
            className="hidden lg:flex landscape:flex w-2 lg:w-3 cursor-col-resize hover:bg-indigo-300 active:bg-indigo-400 items-center justify-center shrink-0 z-20 transition-colors"
          >
            <div className="w-1 h-12 bg-slate-300 rounded-full"></div>
          </div>
        )}

        <div className={`w-full ${isSingleColumn ? 'max-w-5xl h-full' : 'flex-1 h-[55%] lg:h-full landscape:h-full'} flex flex-col bg-slate-100 md:rounded-2xl shadow-inner overflow-y-auto p-3 md:p-6 scrollbar-thin relative`}>
          <div className="flex-1 pb-4">
            {questionsData.map((q, idx) => (
              <div key={idx} className={`relative ${idx !== 0 ? 'mt-8 pt-8 border-t-2 border-slate-200/60' : ''}`}>
                
                {/* Options Panel Rendered Inside Sticky Wrapper */}
